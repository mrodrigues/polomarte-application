...
describe Projeto do
  ...
  it "deve possuir método #concluído" do
    projeto.should respond_to :concluido?
  end
  ...
  describe "#concluido?" do
    ...
    it "não deve estar concluído se suas tarefas não estiverem concluídas" do
      projeto.tarefas.each { |tarefa| tarefa.concluida = false }
    ...
    it "deve estar concluído apenas se todas as suas tarefas estiverem concluídas" do
      projeto.tarefas.each { |tarefa| tarefa.concluida = true }
